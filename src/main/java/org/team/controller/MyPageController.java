...
public class MyPageController {
	...
	public void mypage(@RequestParam(name = "id") String id,
			@RequestParam(name = "pageNum", defaultValue = "1") Integer pageNum,
			@RequestParam(name = "amount", defaultValue = "10") Integer amount, Model model) {
		amount = 5;
	...
	public void replyManagement(Model model, 
								HttpSession session,
								@RequestParam(name = "pageNum", defaultValue = "1") Integer pageNum) {
		MemberVO mVo = (MemberVO) session.getAttribute("mVO");
		orderCriteria cri = new orderCriteria(pageNum, 5);
		int user_id = mVo.getId();
		int total = orderservice.getReplyTotal(user_id);

		PageDTO pageDTO = new PageDTO(cri, total);

		List<ReplyVO> list = orderservice.replyManagement(user_id, cri);
		log.info(pageDTO);

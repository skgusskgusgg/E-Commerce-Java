...
$(document).ready(function(){
	...
	function updateReviewProduct(event){
		
		var updateReviewHtmlString = "";
		updateReviewHtmlString += "<div class='flex-w flex-m p-t-50 p-b-23'>";
		updateReviewHtmlString += "<span class='stext-102 cl3 m-r-16'> Your Rating </span>";
		updateReviewHtmlString += "<span class='wrap-rating-update fs-18 cl11 pointer'>";
		for(i=0; i<parseInt($(event.target).parent().parent().parent().parent().find('.product-reply-star').val());i++){
			updateReviewHtmlString += "<i class='item-rating pointer zmdi zmdi-star'></i> ";
		}
		for(i=0; i<5-parseInt($(event.target).parent().parent().parent().parent().find('.product-reply-star').val());i++){
			updateReviewHtmlString += "<i class='item-rating pointer zmdi zmdi-star-outline'></i> ";
		}
		updateReviewHtmlString += "<input class='review-product-star dis-none' type='number' name='rating' value='"+$(event.target).parent().parent().parent().parent().find('.product-reply-star').val()+"'></span></div>";
		updateReviewHtmlString += "<input class='product-reply-id dis-none' type='number' name='rating' value='"+$(event.target).parent().parent().parent().parent().find('.product-reply-id').val()+"'>";
		updateReviewHtmlString += "<div class='row p-b-25'><div class='col-sm-6 p-b-5'><label class='stext-102 cl3' for='name'>Name</label>";
		updateReviewHtmlString += "<input class='review-product-writer-name size-111 bor8 stext-102 cl2 p-lr-20' type='text' name='name' value='"+$(event.target).parent().parent().parent().parent().find('.product-reply-writer-name').text()+"'></div>";
		updateReviewHtmlString += "<div class='col-12 p-b-5'><label class='stext-102 cl3' for='review'>Your review</label>";
		updateReviewHtmlString += "<textarea class='review-product-content size-110 bor8 stext-102 cl2 p-lr-20 p-tb-10' name='review'>"+$(event.target).parent().parent().parent().parent().find('.product-reply-content').text()+"</textarea></div></div>";
		updateReviewHtmlString += "<div style='display: flex; justify-content: space-evenly;'><span class='replyUpdateBtn flex-c-m stext-101 cl0 size-112 bg7 bor11 hov-btn3 p-lr-15 trans-04 m-b-10 pointer' onclick='replyUpdate(event)'>Update</span>";
		updateReviewHtmlString += "<span class='flex-c-m stext-101 cl0 size-112 bg7 bor11 hov-btn3 p-lr-15 trans-04 m-b-10 pointer' onclick='replyUpdateCancel(event)'>Cancel</span></div>";
		$(event.target).parent().parent().parent().parent().html(updateReviewHtmlString);
	...
	function replyUpdate(event){
		Swal.fire({
			  ...
			  if (result.isConfirmed) {
				  var writer = $(event.target).parent().parent().find('.review-product-writer-name').val();
					var content = $(event.target).parent().parent().find('.review-product-content').val();
					var star = parseInt($(event.target).parent().parent().find('.review-product-star').val());
					var reply_id = parseInt($(event.target).parent().parent().find('.product-reply-id').val());
	...
	function replyUpdateCancel(event){
		pagePrepare();
	}
		...
		$(data).find('item').each((index, item) => {
			...
			if(${mVO.id}){
				if(${mVO.id}==parseInt($(item).find("user_id").text())){
					...
					reviewString += "<span class='flex-c-m stext-101 cl5 size-127 bg2 bor18 hov-btn3 p-lr-15 pointer trans-04' onclick='updateReviewProduct(event)' >";
					reviewString += " 수정 </span></span></span>";
					reviewString += "<span class='m-l-8'> <span class='cl4'>";
